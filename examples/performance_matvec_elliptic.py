...
def simulation_ht(
    degree: int, nbel: int, quadclass: Literal["gauss", "weighted"], ndim=3
):
    NTIMES = 10
    ...
    nbctrlpts_total = quadrule.nbctrlpts**ndim
    nbquadpts_total = quadrule.nbquadpts**ndim

    rng = np.random.default_rng(0)
    v = rng.random(nbctrlpts_total)

    coefficients = np.ones(nbquadpts_total)
    time_sum = 0.0
    ...
    time_capa = time_sum / NTIMES

    coefficients = np.ones((3, 3, nbquadpts_total))
    time_sum = 0.0
    for _ in range(NTIMES):
        start = process_time()
        ...
        finish = process_time()
        time_sum += finish - start
    time_cond = time_sum / NTIMES

    return time_capa, time_cond
...
for n in nbel_list:
    for p in degree_list:
        t1, t2 = simulation_ht(degree=p, nbel=n, quadclass="weighted")
        results.append(
            {"degree": p, "nbel": n, "time_capacity": t1, "time_conductivity": t2}
        )
        filename = FOLDER / "cpu_time_elliptic_matvec.csv"
